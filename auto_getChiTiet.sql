...
-- 1. Tạo Procedure
CREATE OR REPLACE PROCEDURE P_CREATE_ONEBSS IS
    v_month VARCHAR2(6);
    v_table_name VARCHAR2(30);
    v_date_str VARCHAR2(10);
BEGIN
    SELECT TO_CHAR(ADD_MONTHS(SYSDATE, -1), 'YYYYMM') INTO v_month FROM dual;
    v_table_name := 'ONEBSS_' || v_month;
    v_date_str := '01/' || SUBSTR(v_month, 5, 2) || '/' || SUBSTR(v_month, 1, 4);
    ...
    EXECUTE IMMEDIATE '
        ...
        std_onebss AS (
            SELECT b.ngay_ins, b.tthd_id, b.loaitb_id, a.ma_gd, b.hdtb_id, b.thuebao_id, b.ma_tb, a.loaihd_id, b.kieuld_id, 
                   b.donvi_id donvi_tt_id, a.ngay_yc, b.ngay_ht, a.ctv_id, a.nhanviengt_id, a.nhanvien_id,
                   d.ngay_tt, d.ngay_hd, d.seri, d.soseri, c.khoanmuctt_id, d.thungan_tt_id, d.ht_tra_id, 
                   d.kenhthu_id, d.trangthai, cq.tenchuquan, cq.chuquan_id,
            ...
            WHERE (
                TO_CHAR(b.ngay_ins, ''YYYYMM'') = TO_CHAR(ADD_MONTHS(TO_DATE(''' || v_date_str || ''', ''DD/MM/YYYY''), -1), ''YYYYMM'')
                OR (
                    d.ngay_tt < TRUNC(TO_DATE(''' || v_date_str || ''', ''DD/MM/YYYY''), ''MONTH'')
                    AND NVL(ngay_ht, TO_DATE(''' || v_date_str || ''', ''DD/MM/YYYY'')) >= TRUNC(TO_DATE(''' || v_date_str || ''', ''DD/MM/YYYY''), ''MONTH'')
                    AND TO_CHAR(b.ngay_ins, ''YYYYMM'') = TO_CHAR(ADD_MONTHS(TO_DATE(''' || v_date_str || ''', ''DD/MM/YYYY''), -1), ''YYYYMM'')
                )
            )
        ...
        x_onebss AS (
            SELECT a.ngay_ins, a.tthd_id, a.loaihd_id, a.kieuld_id, a.trangthai, a.khoanmuctt_id, dv.dichvuvt_id, 
                   a.loaitb_id, q.loaihinh_tb, a.ma_gd, a.hdtb_id, a.thuebao_id, a.ma_tb, b.MA_LOAIHD, b.TEN_LOAIHD, 
                   c.ten_kieuld, a.ngay_yc, a.ngay_ht, d.ten_nv, m.ten_dv, s.ten_dv pbh, a.donvi_tt_id, 
                   l.ten_dv ten_pb_ttvt, a.ngay_tt, a.ngay_hd, a.seri, a.soseri,
            ...
            GROUP BY a.ngay_ins, a.tthd_id, a.HDTB_ID, dv.dichvuvt_id, q.loaihinh_tb, a.loaitb_id, a.ma_gd, 
                     ...
                     CASE WHEN a.trangthai = 1 THEN ''Da thu tien'' ELSE ''Chua thu tien'' END
        )
...
BEGIN
  DBMS_SCHEDULER.CREATE_JOB (
    job_name        => 'JOB_CREATE_ONEBSS',
    job_type        => 'STORED_PROCEDURE',
    job_action      => 'P_CREATE_ONEBSS',
    start_date      => TO_TIMESTAMP_TZ('2025-06-01 07:00:00 Asia/Ho_Chi_Minh', 'YYYY-MM-DD HH24:MI:SS TZR'),
    repeat_interval => 'FREQ=MONTHLY;BYMONTHDAY=1;BYHOUR=7;BYMINUTE=0;BYSECOND=0',
    enabled         => TRUE,
    comments        => 'Tạo bảng ONEBSS DBHM kế toán chị Hương hàng tháng lúc 7h sáng ngày 1'
  );
END;
/
