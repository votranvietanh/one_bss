select * from (select a.LOAITB_ID,a.LOAIHINH_TB,a.khoanmuctt_id, a.MA_GD, a.HDTB_ID, a.THUEBAO_ID, a.MA_TB,c.ten_kh, a.MA_LOAIHD, a.TEN_LOAIHD, a.TEN_KIEULD, a.NGAY_YC, a.NGAY_HT, a.TEN_NV, a.TEN_DV, a.PBH, a.DONVI_TT_ID, a.TEN_PB_TTVT, a.NGAY_TT, a.NGAY_HD, a.SERI, a.SOSERI, a.TIEN, a.VAT
        ,a.TIEN+a.VAT tong_ps,
         (CASE
                    WHEN 
                         (TO_NUMBER(TO_CHAR(NGAY_TT, 'YYYYMM')) = 202509  or ngay_tt is null )
                ...
                ,
                (CASE
                    WHEN (TO_NUMBER(TO_CHAR(ngay_tt, 'YYYYMM')) <> 202509 and trangthai= 1)
                ...
                (CASE
                    WHEN 
                        (TO_NUMBER(TO_CHAR(ngay_tt, 'YYYYMM')) <> 202509  and trangthai= 1)
        ...
        from  vietanhvh.onebss_202509_v999 a
        ...
        where  a.khoanmuctt_id  in (1,2,3,4,9,17,29)
                and a.tthd_id = 6 and a.dichvuvt_id in (1,4,7,8,9,10,11,12)
                and hdtb_id not in (select hdtb_id from css_hcm.hdtb_dungthu 
                                    where to_number(to_char(NGAY_BD,'yyyymm'))= 202509 and trangthai <> 0 
                                            and to_number(to_char(NGAY_CHUYEN,'yyyymm'))= 202509
                )
                )
                ;
...
--         AND a.loaitb_id not in (35,80,148,350,2116,147,39,290,175,122,319,208,288,373) -- dk chi Huong+319,208: elearning va eticket
         ...
         AND a.khoanmuctt_id in (1,5,52,142) and a.ma_tb in(select ma_tb from vietanhvh.onebss_202509_v999)
         ;


--ChiTiet--

create table onebss_202509 as
WITH ct AS (
    SELECT 
        khoanmuctt_id,
        hdtb_id,
...
std_onebss AS (
    ...
    WHERE 
      (
        TO_CHAR(b.ngay_ht, 'yyyymm') = '202509'
--            or (
--                    d.ngay_tt < trunc(sysdate, 'month')
--                    and nvl(ngay_ht, sysdate) >= trunc(sysdate, 'month')
--                    and TO_CHAR(b.ngay_ht, 'yyyymm') = TO_CHAR(ADD_MONTHS(SYSDATE, -1), 'YYYYMM')
--            )
        )
